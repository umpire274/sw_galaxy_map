...
use sw_galaxy_map_core::db::queries::{UnknownPlanetUpdate, count_unknown_planets, list_unknown_planets_paginated, near_planets_for_unknown_id, update_unknown_planet, near_unknown_planets, find_planet_by_norm};
...
pub fn run(con: &Connection, cmd: &UnknownCmd) -> Result<()> {
    match cmd {
        ...
        UnknownCmd::Near { planet, range, limit } => {
            if *range <= 0.0 {
            ...
            for h in hits {
                println!(
                    ...
                    h.id,
                    h.fid.map(|v| v.to_string()).unwrap_or_else(|| "-".to_string()),
                    h.planet,
                    h.x.map(|v| format!("{v:.2}")).unwrap_or_else(|| "-".to_string()),
                    h.y.map(|v| format!("{v:.2}")).unwrap_or_else(|| "-".to_string()),
                    h.distance,
