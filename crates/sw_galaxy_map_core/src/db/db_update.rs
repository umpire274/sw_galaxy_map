...
fn unknown_seen_key(fid: Option<i64>, planet_norm: &str, x: Option<f64>, y: Option<f64>, reason: &str) -> String {
    match fid {
...
fn sync_unknown_planets(tx: &Transaction<'_>, skipped_rows: &[SkippedPlanetRow]) -> Result<()> {
    ...
    let mut mark_seen = tx.prepare_cached(
        "INSERT OR IGNORE INTO __unknown_seen_keys(seen_key) VALUES (?1)",
    )?;
